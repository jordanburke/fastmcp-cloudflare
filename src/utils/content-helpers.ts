 ...
 */
export async function imageContent(input: { buffer: Buffer | ArrayBuffer } | { url: string }): Promise<ImageContent> {
  ...
  try {
    if ("url" in input) {
      ...
      rawData =
        input.buffer instanceof ArrayBuffer
          ? input.buffer
          : input.buffer.buffer.slice(input.buffer.byteOffset, input.buffer.byteOffset + input.buffer.byteLength)
    ...
    throw new Error(`Failed to process image content: ${error instanceof Error ? error.message : String(error)}`)
  }
}
 ...
 * Supports URL and Buffer inputs (no file system access)
 */
export async function audioContent(input: { buffer: Buffer | ArrayBuffer } | { url: string }): Promise<AudioContent> {
  let rawData: ArrayBuffer
  let mimeType: string
  ...
  try {
    ...
    throw new Error(`Failed to process audio content: ${error instanceof Error ? error.message : String(error)}`)
  }
}
