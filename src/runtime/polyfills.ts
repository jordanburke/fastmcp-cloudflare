...
export const readFile = {
  async readFile(pathOrUrl: string): Promise<Buffer> {
    ...
    if (!pathOrUrl.startsWith('http://') && !pathOrUrl.startsWith('https://')) {
      throw new Error(
        'File system access not available in Workers. Use URLs instead of file paths.'
      );
    }
    ...
    try {
      ...
      throw new Error(
        `Failed to read file from URL: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
};
...
export function getEventEmitter(): typeof WorkersEventEmitter {
  ...
  // In Node.js, try to use native EventEmitter
  try {
